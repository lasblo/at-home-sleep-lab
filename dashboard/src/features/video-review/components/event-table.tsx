...
function DebugInfo({ event }: { event: SleepEvent }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-x-6 gap-y-1 p-3 text-[10px] text-muted-foreground tabular-nums">
      <div>
        Signal: {d.raw_localized.toFixed(3)} &rarr; {d.smoothed.toFixed(3)}
      </div>
      <div>
        Baseline: {d.baseline.toFixed(3)} | Above: {d.above_baseline.toFixed(3)}
      </div>
      <div>
        Peak: {d.normalized_height.toFixed(4)}
        <span
          className={
            d.normalized_height >= 0.02
      ...
      <div>
        ...
        <span
          className={
            d.sv_passed ? "text-severity-normal" : "text-severity-severe"
          }
        >
      ...
      <div>
        Body: {d.body_classification}
        {d.body_reason ? ` (${d.body_reason})` : ""}
      </div>
      <div>
        PLM: {d.plm_eligible ? "eligible" : "ineligible"}
        {d.plm_reject_reason ? ` (${d.plm_reject_reason})` : ""}
      </div>
      {d.interval_to_prev_sec != null && (
        <div>
          ...
          <span
            className={
              d.interval_valid ? "text-severity-normal" : "text-severity-severe"
            }
          >
      ...
      {event.arousal?.has_arousal && (
        <>
          <div>
            HR baseline: {event.arousal.pre_baseline_hr?.toFixed(0)} bpm
          </div>
          <div>
            HR peak: {event.arousal.peak_hr} bpm (+
            {event.arousal.magnitude_bpm?.toFixed(1)} /{" "}
            {((event.arousal.magnitude_pct ?? 0) * 100).toFixed(0)}%)
          </div>
          <div>
            Onset delay: {event.arousal.onset_delay_sec?.toFixed(1)}s |
            Duration: {event.arousal.duration_sec?.toFixed(1)}s
          </div>
          <div>
            Threshold: {event.arousal.threshold_used} | Strict:{" "}
            {event.arousal.strict_threshold_met ? "yes" : "no"}
          </div>
        </>
      )}
    </div>
